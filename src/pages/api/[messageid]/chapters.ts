...
async function getRequest(req: NextApiRequest, res: NextApiResponse) {
    ...
    // Gets every chapter/message with the same seriesid as the given messageid
    const seriesIDQuery = await query(
        `SELECT message, name, messageid FROM chapters WHERE seriesid = (SELECT seriesid FROM chapters WHERE messageid = $1)`,
        [messageid]
    );
...
async function postRequest(req: NextApiRequest, res: NextApiResponse) {
    ...
    const { prompt, storyName, messageid } = req.body;
